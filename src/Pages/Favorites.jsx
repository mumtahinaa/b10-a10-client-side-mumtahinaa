...
const Favorites = () => {
    ...
    useEffect(() => {
      fetch(`http://localhost:4000/favorite/${user?.email}`)
        .then((res) => res.json())
        .then((data) =>{
    ...
    const handleDeleteFavorite = (_id) =>{
        ...
        Swal.fire({
            ...
            if (result.isConfirmed){
                ...
                fetch(`http://localhost:4000/favorite/${_id}`,{
                    method:"DELETE"
                   })
                   .then(res=>res.json())
                   .then(data=>{
    ...
    return (
      ...
      <div className="w-full max-w-7xl">
        ...
        {favoriteMovies.length === 0 ? (
          <p className="text-center text-gray-400 text-2xl">No favorite movies added yet.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {favoriteMovies.map((movie) => (
              <div key={movie._id} className="bg-[#252525] p-4 rounded-lg shadow-lg">
                <img
                  src={movie.Poster}
                  alt={movie.Title}
                ...
                <div className="flex items-center gap-2 mt-2">
                  <Rating initialValue={movie.Rating} readonly size={20} className='rating-container' />
