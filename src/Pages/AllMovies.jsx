       ...
       {allMovies.length>0 ?  (allMovies.map((movie) => (
          <div key={movie._id} className="bg-[#1A1A1A] p-3 rounded-md shadow-lg border border-[#9B5DE5] flex items-center gap-4 transform transition duration-300 hover:scale-105">
            ...
            <div className="w-1/2  ">
              ...
              <p className="text-white  xl:text-sm lg:text-xs text-sm font-pacifico">{movie.genre}</p>
              <p className="text-white xl:text-sm lg:text-xs text-sm">Duration: {movie.duration} min</p>
              <p className="text-white xl:text-sm lg:text-xs text-sm">Release Year: {movie.releaseYear}</p>
              <p className="text-yellow-400 xl:text-sm lg:text-xs text-sm font-bold">Rating: {movie.rating}</p>
              <button
